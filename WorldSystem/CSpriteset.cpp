...
bool STilesetSprites::save(FileSystem::CFileStream& file)
{
	...
	Common::scrambleString(ma_password, SPRITESET_PASSWORD_LENGTH);
	Common::xorData((unsigned int*)ma_password, SPRITESET_PASSWORD_LENGTH / sizeof(int), SPRITESET_PASSWORD_KEY);
	file.write(ma_password, sizeof(char), SPRITESET_PASSWORD_LENGTH);
	// decrypt again
	Common::xorData((unsigned int*)ma_password, SPRITESET_PASSWORD_LENGTH / sizeof(int), SPRITESET_PASSWORD_KEY);
...
bool STilesetSprites::open(FileSystem::CFileStream& file, char* password)
{
	int version = 0;
	file.read(&version, sizeof(int), 1);

	if (version > TSFV_050713)
	{
		file.read(ma_password, sizeof(char), SPRITESET_PASSWORD_LENGTH);
		Common::xorData((unsigned int*)ma_password, SPRITESET_PASSWORD_LENGTH / sizeof(int), SPRITESET_PASSWORD_KEY);
		if (password)
		{
